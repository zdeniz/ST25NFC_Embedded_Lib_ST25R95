...
static const uint8_t* ndefRtdWlcStatusInfoGetPayloadItem(const ndefType* type, ndefConstBuffer* bufItem, bool begin)
{
    ...
    switch (item)
    {
    case 0: /* Contol Byte 1 */
        bufItem->buffer = &ndefData->controlByte1;
        item++;
        break;
    case 1: /* Battery Level */
        if (ndefData->controlByte1 & NDEF_WLC_STATUSINFO_CONTROLBYTE1_BATTERY_LEVEL_MASK)
        {
    ...
    case 2: /* Receive Power */
        if (ndefData->controlByte1 & NDEF_WLC_STATUSINFO_CONTROLBYTE1_RECEIVE_POWER_MASK)
        {
    ...
    case 3: /* Receive Voltage */
        if (ndefData->controlByte1 & NDEF_WLC_STATUSINFO_CONTROLBYTE1_RECEIVE_VOLTAGE_MASK)
        {
    ...
    case 4: /* Receive Current */
        if (ndefData->controlByte1 & NDEF_WLC_STATUSINFO_CONTROLBYTE1_RECEIVE_CURRENT_MASK)
        {
    ...
    case 5: /* Receive Current */
        if (ndefData->controlByte1 & NDEF_WLC_STATUSINFO_CONTROLBYTE1_TEMPERATURE_BATTERY_MASK)
        {
    ...
    case 6: /* Receive Current */
        if (ndefData->controlByte1 & NDEF_WLC_STATUSINFO_CONTROLBYTE1_TEMPERATURE_WLCL_MASK)
        {
    ...
    case 7: /* Receive Current */
        if (ndefData->controlByte1 & NDEF_WLC_STATUSINFO_CONTROLBYTE1_RFU_MASK)
        {
    ...
    case 8: /* Receive Current */
        if (ndefData->controlByte1 & NDEF_WLC_STATUSINFO_CONTROLBYTE1_CONTROL_BYTE_2_MASK)
        {
...
static const uint8_t* ndefRtdWlcPollInfoGetPayloadItem(const ndefType* type, ndefConstBuffer* bufItem, bool begin)
{
    ...
    if (item < (sizeof(ndefTypeRtdWlcPollInfo)/sizeof(uint8_t)))
    {
        /* ndefTypeRtdWlcPollInfo contains bytes only, so access its content by indexing the array */
        bufItem->buffer = &((const uint8_t*)ndefData)[item];
        bufItem->length = sizeof(uint8_t);
    }
    else
    {
