...
ReturnCode ndefT2TPollerReadBytes(ndefContext *ctx, uint32_t offset, uint32_t len, uint8_t *buf, uint32_t *rcvdLen)
{
    ...
    else
    {
        do {
            ...
            {
                numOfValidBlocks = -(uint8_t)blockAddr;
...
static ReturnCode ndefT2TPollerSplitIntoAvailableAreas(ndefContext *ctx, uint32_t offset, uint32_t len, uint32_t *physOffset, uint32_t *maxLen)
{
    ...
    if( (ctx == NULL) || !ndefT2TisT2TDevice(&ctx->device) || (physOffset == NULL) || (maxLen == NULL) )
    {
        return ERR_PARAM;
    }
...
/*******************************************************************************/
static ReturnCode ndefT2TPollerWriteBytesToAvailableAreas(ndefContext *ctx, uint32_t offset, const uint8_t *buf, uint32_t len)
{
    ReturnCode ret;
    uint32_t curOffset;
    uint32_t curPhyOffset;
    ...
    {
        (void)ndefT2TPollerSplitIntoAvailableAreas(ctx, curOffset, remainingLen, &curPhyOffset, &maxLen);
        ret = ndefT2TPollerWriteBytes(ctx, curPhyOffset, &buf[len - remainingLen], maxLen);
...
ReturnCode ndefT2TPollerWriteRawMessageLen(ndefContext *ctx, uint32_t rawMessageLen)
{
    ...
    if( (rawMessageLen != 0U) && ((ctx->messageOffset + rawMessageLen) < ctx->areaLen) )
    {
        ...
        dataIt++;
        ret = ndefT2TPollerWriteBytesToAvailableAreas(ctx, ctx->messageOffset + rawMessageLen, buf, dataIt);
        if( ret != ERR_NONE )
        {
            return ret;
        }
    ...
    ret = ndefT2TPollerWriteBytesToAvailableAreas(ctx, ctx->subCtx.t2t.offsetNdefTLV, buf, dataIt);
    return ret;
}
...
ReturnCode ndefT2TPollerWriteRawMessage(ndefContext *ctx, const uint8_t *buf, uint32_t bufLen)
{
    ...
    if( bufLen != 0U )
    {
       /* TS T2T v1.0 7.5.3.5: write new NDEF message */
        ret = ndefT2TPollerWriteBytesToAvailableAreas(ctx, ctx->messageOffset, buf, bufLen);
        if (ret != ERR_NONE)
        {
            /* Conclude procedure */
            ctx->state = NDEF_STATE_INVALID;
...
ReturnCode ndefT2TPollerSetReadOnly(ndefContext *ctx)
{
    ...
    ret = ndefT2TPollerWriteBytes(ctx, NDEF_T2T_STATLOCK_OFFSET, staticBits, sizeof(staticBits));
    if( ret != ERR_NONE )
    {
        return ret;
    }
    ...
    if( ctx->areaLen > NDEF_T2T_STATIC_MEM_SIZE )
    {
        ...
        ret = ndefT2TPollerWriteBytes(ctx, ctx->subCtx.t2t.dynLockFirstByteAddr, dynLockBits, i);
        if( ret != ERR_NONE )
        {
            return ret;
        }
